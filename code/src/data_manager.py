...
class DataManager:
    def __init__(self, interim_path, pairs_path, raw_path=None, arguments=None, setup_logging=False):
    ...
    def load_or_prepare(self):
        ...
        if self.raw_path is None:
            logging.info("No raw data path provided, skipping raw data loading.")
        ...
        if self.interim_path.exists():
            self.created_interim = False
            logging.info(f'Loading interim data from {self.interim_path}')
            interim_data = self._load_netcdf(self.interim_path)
        else:
            self.created_interim = True
            logging.info(f'Creating interim data from {self.raw_path}')
            logging.info(f'Using arguments: {self.arguments}')
            interim_data = self._create_interim()
            self._save_netcdf(interim_data, self.interim_path)
        self.interim_data = interim_data
        logging.info(f'Interim data loaded with shape: {interim_data.sizes}')
        logging.info(f'Interim data variables: {interim_data.data_vars}')

        # Step 3: Check or create model input
        if self.pairs_path.exists():
            self.created_pairs = False
            logging.info(f'Loading model input data from {self.pairs_path}')
            pairs = self._load_pickle(self.pairs_path)
        else:
            self.created_pairs = True
            logging.info(f'Creating mode input data from {self.interim_path}')
            logging.info(f'Using arguments: {self.arguments}')
            pairs = self._create_model_input()
            self._save_pickle(pairs, self.pairs_path)
        self.pairs = pairs
        logging.info(f'Model input data loaded with {len(pairs)} pairs.')
    ...
    def _create_interim(self):
        ...
        logging.info("Processing raw data to interim.")
    ...
    def _create_model_input(self):
        # Replace with your actual logic
        logging.info("Transforming interim data to feature label pairs.")
    ...
    def _apply_subsetting(self, raw_data, args):
        ...
        logging.info("Applying processing to raw data.")
        ...
        #     logging.info(f"Subsetting time from {args['subset_time'][0]} to {args['subset_time'][1]}")
        ...
        if args['subset_region'] == 'Arctic':
            ...
            interim_data = interim_data.where(interim_data['nav_lat'] < -60, drop=True)
        logging.info(f"Subsetting region to {args['subset_region']}")
    ...
    def _make_feature_label_pairs(self, interim_path, args):
        ...
        logging.info(f"Making feature-label pairs from {interim_path}")
